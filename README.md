...
A full multiplayer gaming platform built in Java, using BlueJ, as a school project. 46+ playable games, ELO ratings, a party system, tournaments, and the ability for anyone to host their own server.
...
**More single-player (no server needed):** Minesweeper, Sudoku, Simon Says, Whack-a-Mole, Gem Match (an original match-3 - swap gems, clear lines, chain cascades for bonus score), Maze Chase (an original maze-and-chasers game - clear every dot, dodge the chasers, grab a power pellet to turn the tables for a few seconds), Brick Breaker (bounce the ball off the paddle to smash the wall of bricks), Flappy Bird (tap to flap upward, gravity pulls you back down, thread the gaps between scrolling pipes), Galaxy Defender (an original fixed-shooter - clear each marching wave of enemies before they reach your line, wave after wave), Word Guess (guess the hidden 5-letter word in 6 tries with letter-position feedback), Bubble Shooter (aim and fire colored bubbles into a hex grid, matching 3+ to pop them and drop anything left floating), Lights Out (click a light to toggle it and its neighbors, turn every light off to solve the puzzle), Peg Solitaire (the classic cross-shaped board - jump pegs over each other until none can jump again, ideally down to just one left in the center), Klondike Solitaire (the traditional card-solitaire patience game - build the four suit foundations up from Ace to King), and Yahtzee (roll five dice up to three times a turn, holding what you like, and score into one of 13 categories).
