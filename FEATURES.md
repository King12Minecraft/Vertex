...
- **First game migrated: Tic-Tac-Toe Practice Mode.** Its existing AI (`TicTacToeAI.pickMove`) is now wrapped as a registered `BotStrategy` rather than called directly, with a separate, deliberately simple "pick any open cell at random" fallback — proving the architecture end-to-end on a real game with zero behavior change, verified by 260 automated checks (exact-decision regression tests plus 200 simulated full games).
