...
import games.CheckersMatch;
import games.ConnectFourMatchManager;
import games.ConnectFourMatch;
import games.TicTacToeMatch;
...
public class ClientHandler implements Runnable
{
    ...
    private CheckersMatch currentCheckersMatch;
    ...
    private TetrisDuelMatch currentTetrisDuelMatch;
    private TetrisDuelMatchManager tetrisDuelMatchManager;
    private FusionGridMatch currentFusionGridMatch;
    ...
    private Message handle(Message request)
    {
        ...
        if (request.getType() == MessageType.CHECKERS_FIND_MATCH_REQUEST) return handleCheckersFindMatch();
        if (request.getType() == MessageType.CHECKERS_LEAVE_QUEUE_REQUEST) return handleCheckersLeaveQueue();
        if (request.getType() == MessageType.CHECKERS_MOVE_REQUEST) return handleCheckersMove(request);
        ...
        if (request.getType() == MessageType.TETRISDUEL_FIND_MATCH_REQUEST) return handleTetrisDuelFindMatch();
        if (request.getType() == MessageType.TETRISDUEL_LEAVE_QUEUE_REQUEST) return handleTetrisDuelLeaveQueue();
        if (request.getType() == MessageType.TETRISDUEL_ACTION_REQUEST) return handleTetrisDuelAction(request);
        if (request.getType() == MessageType.FUSIONGRID_FIND_MATCH_REQUEST) return handleFusionGridFindMatch();
        if (request.getType() == MessageType.FUSIONGRID_LEAVE_QUEUE_REQUEST) return handleFusionGridLeaveQueue();
        if (request.getType() == MessageType.FUSIONGRID_PLACE_REQUEST) return handleFusionGridPlace(request);
    ...
    private Message handleGamePlayed(Message request)
    {
        if (loggedInAccountId != null)
        {
            ...
            {
                // Shape 1 (EconomyKernel.awardCompletion): covers every other offline/
                // practice game - including Snake, folded into this generic path rather
                // than its own special case now that its formula lives in
                // EconomyConfig.getPracticeReward like every other game's - via
                // getPracticeReward's per-game formula table. A safe default rather
                // than a hand-maintained id list that has to be kept in sync with that
                // table by hand: getPracticeReward already returns 0 (a no-op) for any
                // gameId it has no formula for, so this never double-rewards an online
                // game that also calls GAME_PLAYED_REQUEST for history tracking
                // (zombie-survival/space-battle above, and every other online game,
                // none of which appear in getPracticeReward's table).
                EconomyKernel.awardCompletion(economyManager, this, gameId, request.getScore());
            }
        }
        return null;
    }
    ...
    private Message handleCheckersFindMatch()
    {
        if (loggedInUsername != null) checkersMatchManager.findMatch(this);
        return null;
    }

    private Message handleCheckersLeaveQueue()
    {
        checkersMatchManager.cancelWaiting(this);
    ...
    private Message handleCheckersMove(Message request)
    {
        if (currentCheckersMatch != null)
        {
            currentCheckersMatch.makeMove(this, request.getCellIndex(), request.getChessToSquare());
        }
        return null;
    }
    ...
    private Message handleSquareWarsFindMatch()
    {
        if (loggedInUsername != null) squareWarsMatchManager.findMatch(this);
        return null;
    }

    private Message handleSquareWarsLeaveQueue()
    {
        squareWarsMatchManager.cancelWaiting(this);
    ...
    private Message handleSquareWarsClaim(Message request)
    {
        if (currentSquareWarsMatch != null)
        {
    ...
    private Message handleMemoryFindMatch()
    {
        if (loggedInUsername != null) memoryMatchMatchManager.findMatch(this);
        return null;
    }

    private Message handleMemoryLeaveQueue()
    {
        memoryMatchMatchManager.cancelWaiting(this);
    ...
    private Message handleMemoryFlip(Message request)
    {
        if (currentMemoryMatchMatch != null)
        {
    ...
    private Message handleTetrisDuelFindMatch()
    {
        if (loggedInUsername != null) tetrisDuelMatchManager.findMatch(this);
        return null;
    }

    private Message handleTetrisDuelLeaveQueue()
    {
        tetrisDuelMatchManager.cancelWaiting(this);
    ...
    /** Reuses getChatText() for the action name (LEFT/RIGHT/ROTATE/SOFT_DROP/HARD_DROP). */
    private Message handleTetrisDuelAction(Message request)
    {
        if (currentTetrisDuelMatch != null)
        {
            currentTetrisDuelMatch.applyAction(this, request.getChatText());
        }
        return null;
    }
    ...
    private Message handleFusionGridFindMatch()
    {
        if (loggedInUsername != null) fusionGridMatchManager.findMatch(this);
        return null;
    }

    private Message handleFusionGridLeaveQueue()
    {
        fusionGridMatchManager.cancelWaiting(this);
    ...
    private Message handleFusionGridPlace(Message request)
    {
        if (currentFusionGridMatch != null)
        {
            currentFusionGridMatch.placeTile(this, request.getCellIndex());
        }
        return null;
    }
    ...
    private Message handleTypingDuelFindMatch()
    {
        if (loggedInUsername != null) typingDuelMatchManager.findMatch(this);
        return null;
    }

    private Message handleTypingDuelLeaveQueue()
    {
        typingDuelMatchManager.cancelWaiting(this);
    ...
    /** Reuses getChatText() for the player's currently-typed text. */
    private Message handleTypingDuelProgress(Message request)
    {
        if (currentTypingDuelMatch != null)
        {
