...
- **`GameLauncher.java`** — the plugin **launch** dispatch, and the mandatory
  ...
  gate since there's nothing to preview yet. 40 games so far are the exceptions
  ...
  Among Us, and Fight Arena - every offline/single-player game in the whole
  catalog, both games with `SpectateDialog`/tournament support, and three
