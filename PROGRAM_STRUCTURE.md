...
- **`GameLauncher.java`** — the plugin **launch** dispatch, and the mandatory
  ...
  gate since there's nothing to preview yet. 41 games so far are the exceptions
  ...
  Among Us, Fight Arena, and Square Wars - every offline/single-player game in
  the whole catalog, both games with `SpectateDialog`/tournament support, and
  four converted online-multiplayer games. The remaining online-multiplayer
  games still open their own window until
