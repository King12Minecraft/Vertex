...
- **`GameLauncher.java`** — the plugin **launch** dispatch, and the mandatory
  ...
  gate since there's nothing to preview yet. 44 games so far are the exceptions
  ...
  Among Us, Fight Arena, Square Wars, Trivia Blitz, Air Hockey, and Snake
  Arena - every offline/single-player game in the whole catalog, both games
  with `SpectateDialog`/tournament support, and seven converted
  online-multiplayer games. The remaining online-multiplayer games still open
