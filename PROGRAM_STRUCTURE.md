...
- **`GameLauncher.java`** — the plugin **launch** dispatch, and the mandatory
  ...
  gate since there's nothing to preview yet. 47 games so far are the exceptions
  ...
  Tetris Duel, Fusion Grid, and Card Rush - every offline/single-player game in
  the whole catalog, both games with `SpectateDialog`/tournament support, and
  ten converted online-multiplayer games. The remaining online-multiplayer
